...
################################################################################
# This file is taken from Chun Shen's iEBE-MUSIC framework                     #
################################################################################
...
def _iter_cronos_events(h5f):
    """Yield (label, event_group) for every event in a CRONOS merged database.

    Supports two layouts:
    - CRONOS merged DB (event_database.py build):
        /job_N/event_M/<datasets>
    - Legacy flat layout:
        /event_N/<datasets>  or  /spvn_results_N/<datasets>
    """
    import re
    _JOB_RE = re.compile(r'^job_(\d+)$')
    job_keys = [k for k in h5f.keys() if _JOB_RE.match(k)]
    if job_keys:
        for job_name in sorted(job_keys, key=lambda k: int(_JOB_RE.match(k).group(1))):
            job_group = h5f[job_name]
            for event_name in sorted(
                [k for k in job_group.keys() if k.startswith('event_')],
                key=lambda k: int(k.split('_')[1])
            ):
                label = f"{job_name}/{event_name}"
                yield label, job_group[event_name]
        return
    # Legacy flat layout
...
eventIter = list(_iter_cronos_events(h5_data))

outdata = {}
outdata["global"] = {}

for ievent, (event_i, eventGroup) in enumerate(eventIter):
    if ievent % 100 == 0:
...
print("nev = {}".format(len(eventIter)))
