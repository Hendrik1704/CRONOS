...
class TestAnalyzeError:
    ...
    def test_analyze_subprocess_error(self):
        ...
        subprocess_error = subprocess.CalledProcessError(
            1, "kompost.x", "execution failed"
        )

        analysis = analyze_error(subprocess_error, "KoMPoST")

        assert (
            analysis["is_resource_related"] is False
        )  # Based on actual implementation
        assert (
            "kompost" in analysis["enhanced_message"].lower()
            or "command" in analysis["enhanced_message"].lower()
        )
        assert "original_exception" in analysis
    ...
    def test_analyze_file_not_found_error(self):
        ...
        assert (
            "executable" in analysis["enhanced_message"].lower()
            or "smash" in analysis["enhanced_message"].lower()
        )
        assert (
            analysis["is_resource_related"] is False
        )  # Based on actual implementation
    ...
    def test_analyze_generic_error(self):
        ...
        assert (
            analysis["is_memory_related"] is False
            or analysis["is_memory_related"] is None
        )
