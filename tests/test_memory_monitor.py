...
from utilities.memory_monitor import (
    monitor_memory,
...
class TestMemoryMonitor:
    ...
    def test_monitor_memory_basic(self, mock_process_iter, mock_virtual_memory, mock_sleep):
    ...
    def test_check_cronos_processes(self, mock_process_iter):
        ...
        mock_proc1 = Mock()
        mock_proc1.info = {
            'name': 'python', 'pid': 1234, 'ppid': 1000, 
            'memory_percent': 5.0, 'create_time': 1234567890,
            'cmdline': ['python', 'cronos_simulation.py'],
            'memory_info': Mock(rss=100*1024*1024)  # 100MB
        }
        mock_proc2 = Mock()
        mock_proc2.info = {
            'name': 'music.x', 'pid': 5678, 'ppid': 1234,
            'memory_percent': 15.0, 'create_time': 1234567900,
            'cmdline': ['./music.x'],
    ...
    def test_monitor_memory_with_logging(self, mock_virtual_memory, mock_sleep):
        ...
        with patch('utilities.memory_monitor.psutil.process_iter') as mock_process_iter:
