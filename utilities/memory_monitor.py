...
def monitor_memory(interval=5, threshold=80, log_file=None):
    ...
    if log_file:
        ...
        log_handle.write("timestamp,system_percent,system_available_gb,process_count,high_memory_processes\n")
    ...
    try:
        while True:
            ...
            for proc in psutil.process_iter(['pid', 'name', 'memory_percent']):
                try:
                    if proc.info['memory_percent'] > 5:  # More than 5% of system memory
                        high_mem_processes.append(f"{proc.info['name']}({proc.info['pid']}):{proc.info['memory_percent']:.1f}%")
            ...
            status_color = "🔴" if memory.percent > threshold else "🟢" if memory.percent < 50 else "🟡"
            print(f"{status_color} {timestamp} | Memory: {memory.percent:5.1f}% | "
            ...
            if memory.percent > threshold:
                ...
                if high_mem_processes:
                    print(f"  📊 Top memory consumers: {', '.join(high_mem_processes[:3])}")
            
            # Log to file if specified
            if log_handle:
                log_handle.write(f"{timestamp},{memory.percent},{memory.available/1024/1024/1024},"
...
def check_cronos_processes():
    ...
    for proc in psutil.process_iter(['pid', 'ppid', 'name', 'cmdline', 'memory_info', 'memory_percent', 'create_time']):
        try:
            ...
            if any(keyword in cmdline.lower() for keyword in ['cronos', 'run_simulations']):
            ...
            elif any(keyword in name for keyword in ['music', 'smash', 'kompost', 'iss']) or \
                 any(keyword in cmdline.lower() for keyword in ['musichydro', 'smash', 'kompost.exe', 'iss.exe']):
    ...
    if all_processes:
        print(f"Found {len(cronos_processes)} Python processes and {len(external_processes)} external processes:")
        print(f"Total memory usage: {total_memory:.1f} MB ({total_memory/1024:.1f} GB)\n")
        
        if cronos_processes:
            print("📋 CRONOS Python Processes:")
            for proc in sorted(cronos_processes, key=lambda x: x['memory_mb'], reverse=True):
                print(f"  PID {proc['pid']:6} | {proc['name']:15} | "
                      f"{proc['memory_mb']:8.1f} MB ({proc['memory_percent']:4.1f}%)")
            print()
        
        if external_processes:
            print("⚡ External Physics Codes:")
            for proc in sorted(external_processes, key=lambda x: x['memory_mb'], reverse=True):
                status_icon = "🔥" if proc['memory_mb'] > 8000 else "⚡"
                print(f"  {status_icon} PID {proc['pid']:6} | {proc['name']:15} | "
                      f"{proc['memory_mb']:8.1f} MB ({proc['memory_percent']:4.1f}%)")
...
def main():
    ...
    # Monitor command
    monitor_parser = subparsers.add_parser('monitor', help='Start real-time memory monitoring')
    monitor_parser.add_argument('--interval', type=int, default=5, help='Monitoring interval in seconds (default: 5)')
    monitor_parser.add_argument('--threshold', type=int, default=80, help='Warning threshold percentage (default: 80)')
    monitor_parser.add_argument('--log', type=str, help='Log file to save memory usage data')
    
    # Check processes command
    check_parser = subparsers.add_parser('check', help='Check current CRONOS processes')
    
    # System info command
    info_parser = subparsers.add_parser('info', help='Show system memory information')
    ...
    if args.command == 'monitor':
        ...
        if memory.percent > 90:
            print("  🔴 Critical memory usage - consider stopping non-essential processes")
