...
def convert_oscar_streaming(
    ...
    with open(input_path, "r", buffering=buffer_size) as infile, open(
        output_path, "w", buffering=buffer_size
    ) as outfile:
            ...
            if is_event_boundary(line):
                ...
                if current_event_particles:
                    ...
                    for particle in current_event_particles:
                        ...
                        wiggle_x = particle["x"] + np.random.uniform(
                            -1e-5, 1e-5
                        )
                        wiggle_y = particle["y"] + np.random.uniform(
                            -1e-5, 1e-5
                        )
                        wiggle_z = particle["z"] + np.random.uniform(
                            -1e-5, 1e-5
                        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Convert OSCAR1999A to OSCAR2013 format."
    )
    parser.add_argument("pdgPath", type=Path, help="Path to pdg-SMASH.dat file")
    parser.add_argument(
        "inputFilePath", type=Path, help="Path to OSCAR1999A input file"
    )
    parser.add_argument(
        "outputFilePath", type=Path, help="Directory to save OSCAR2013 file"
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    num_events = convert_oscar_streaming(
        args.inputFilePath, pdg_df, args.outputFilePath
    )
