...
class HDF5Extractor:
    ...
    def __init__(self, h5_file_path: str, output_dir: str = "extracted_data", 
                 log_level: str = "INFO"):
        """
    ...
    def _validate_inputs(self) -> None:
        ...
        if not self.h5_file_path.suffix.lower() in ['.h5', '.hdf5']:
            self.logger.warning(f"File extension {self.h5_file_path.suffix} may not be HDF5")
    ...
    def scan_h5_file(self) -> Tuple[List[Dict], List[Dict]]:
        ...
        def collect_info(name, obj):
            if isinstance(obj, h5py.Dataset):
                ...
                self.logger.debug(f"Found dataset: {name}, shape: {obj.shape}, dtype: {obj.dtype}")
                ...
                groups.append(info)
                self.logger.debug(f"Found group: {name}, items: {len(obj.keys())}")
        ...
        self.logger.info(f"Scan complete: {len(datasets)} datasets, {len(groups)} groups")
    ...
    def display_file_contents(self) -> None:
        ...
        print(f"{Colors.BLUE}Total datasets: {len(self.datasets_info)}{Colors.RESET}")
        print(f"{Colors.BLUE}Total groups: {len(self.groups_info)}{Colors.RESET}\n")
        
        # Display groups
        if self.groups_info:
            print(f"{Colors.BOLD}{Colors.YELLOW}Groups:{Colors.RESET}")
            for i, group in enumerate(self.groups_info):
                print(f"  {Colors.MAGENTA}G{i:2d}{Colors.RESET}: {group['name']} "
        ...
        if self.datasets_info:
            ...
            for i, dataset in enumerate(self.datasets_info):
                size_str = f"{dataset['size_mb']:.2f} MB" if dataset['size_mb'] > 0.1 else f"{dataset['size']} elements"
                print(f"  {Colors.GREEN}{i:2d}{Colors.RESET}: {dataset['name']}")
                print(f"       Shape: {dataset['shape']}, Type: {dataset['dtype']}, Size: {size_str}")
                
                # Show attributes if available
                if dataset['attributes']:
                    print(f"       Attributes: {list(dataset['attributes'].keys())}")
                print()
        
        if not self.datasets_info and not self.groups_info:
            print(f"{Colors.RED}No datasets or groups found in file.{Colors.RESET}")
    ...
    def get_user_selection(self) -> List[int]:
        ...
        """
        if not self.datasets_info:
            print(f"{Colors.RED}No datasets available for selection.{Colors.RESET}")
            return []
        ...
        while True:
            try:
                user_input = input(f"\n{Colors.CYAN}Your selection: {Colors.RESET}").strip()
                ...
                for item in user_input.split(','):
                    ...
                    if item.isdigit():
                        ...
                        if 0 <= idx < len(self.datasets_info):
                            ...
                            print(f"{Colors.RED}Index {idx} out of range (0-{len(self.datasets_info)-1}){Colors.RESET}")
                            continue
                    else:
                        print(f"{Colors.RED}Invalid input: {item}. Please use numbers.{Colors.RESET}")
                        continue
                
                if selected_indices:
                    return sorted(list(set(selected_indices)))  # Remove duplicates and sort
                else:
                    print(f"{Colors.RED}No valid indices selected. Please try again.{Colors.RESET}")
                    
            except KeyboardInterrupt:
                print(f"\n{Colors.YELLOW}Operation cancelled by user.{Colors.RESET}")
                return []
            except Exception as e:
                print(f"{Colors.RED}Error parsing input: {e}. Please try again.{Colors.RESET}")
    ...
    def extract_dataset(self, dataset_info: Dict) -> Optional[str]:
        ...
        try:
            with h5py.File(self.h5_file_path, 'r') as h5_file:
                ...
                output_path = self._save_as_native(dataset, safe_name, dataset_info)
                if not output_path:
                    self.logger.error(f"Could not determine native format for {dataset_name}")
                    return None
    ...
    def _save_as_native(self, dataset, safe_name: str, dataset_info: Dict) -> Optional[str]:
        """
        ...
        try:
            ...
            format_hint = self._detect_native_format(dataset_info, attributes, data)
            ...
            if format_hint == 'config':
                ...
                return self._save_with_smart_detection(data, safe_name, dataset_info, attributes)
                
        except Exception as e:
            self.logger.error(f"Error in native format detection: {e}")
            return None
    
    def _detect_native_format(self, dataset_info: Dict, attributes: Dict, data: Union[np.ndarray, bytes, str, Any]) -> str:
        ...
        if isinstance(data, bytes):
            ...
            return 'source_code' if name.endswith(('.py', '.cpp', '.c', '.h', '.sh')) else 'config'
        ...
        if any(keyword in name for keyword in ['particle', 'track', 'momentum', 'position', 'energy']):
        ...
        if hasattr(data, 'ndim') and data.ndim == 2 and hasattr(data, 'shape') and len(data.shape) > 1 and data.shape[1] > 1:
            return 'text_table'
        
        return 'auto'
    
    def _save_as_config_file(self, data: Union[np.ndarray, str], safe_name: str, attributes: Dict) -> str:
        ...
        with open(output_path, 'w') as f:
            ...
            if isinstance(data, (str, bytes)):
                ...
                f.write(content)
            elif hasattr(data, 'dtype') and data.dtype.kind in ['U', 'S', 'O']:  # String or object data
                ...
                f.write(f"# Original shape: {getattr(data, 'shape', 'scalar')}, type: {getattr(data, 'dtype', type(data))}\n\n")
    ...
    def _save_as_particle_file(self, data: np.ndarray, safe_name: str, attributes: Dict) -> str:
        ...
        with open(output_path, 'w') as f:
            ...
            if data.ndim == 2:
                if data.shape[1] == 3:
                    ...
                    f.write(f"# {' '.join([f'col{i}' for i in range(data.shape[1])])}\n")
    ...
    def _save_as_text_table(self, data: np.ndarray, safe_name: str, attributes: Dict) -> str:
        ...
        with open(output_path, 'w') as f:
            ...
            if data.ndim == 1:
                ...
                f.write(f"# Multi-dimensional data flattened from shape {data.shape}\n")
    ...
    def _save_as_binary_data(self, data: Union[np.ndarray, bytes, str, Any], safe_name: str, attributes: Dict) -> str:
        ...
        if isinstance(data, bytes):
            ...
            try:
                ...
                if '\n' in decoded_text or any(c in decoded_text for c in '0123456789.-+e'):
    ...
    def _save_as_source_file(self, data: np.ndarray, safe_name: str, attributes: Dict) -> str:
    ...
    def _save_with_smart_detection(self, data: Union[np.ndarray, bytes, str], safe_name: str, 
                                 dataset_info: Dict, attributes: Dict) -> str:
        ...
        if isinstance(data, bytes):
            ...
            try:
                ...
                if '\n' in decoded_text or any(c in decoded_text for c in '0123456789.-+e'):
                    ...
                    meta_path = output_path.with_suffix('.dat.meta')
                    with open(meta_path, 'w') as f:
                        f.write(f"# Text data from HDF5 dataset (decoded from bytes)\n")
                        f.write(f"size_bytes: {len(data)}\n")
                        f.write(f"lines: {len(cleaned_lines)}\n")
                        f.write(f"type: decoded_text\n")
                        for key, value in attributes.items():
        ...
        # Handle numpy arrays
            ...
            if data.dtype.kind in ['U', 'S', 'O']:
                ...
                with open(output_path, 'w') as f:
                    if hasattr(data, 'size') and data.size == 1:
                        content = data.item() if hasattr(data, 'item') else str(data)
                        if isinstance(content, bytes):
                            content = content.decode('utf-8', errors='replace')
                        # Clean up leading and trailing whitespace and write
                        cleaned_content = '\n'.join(line.strip() for line in str(content).splitlines())
                        f.write(cleaned_content)
                    else:
                        for item in data.flat if hasattr(data, 'flat') else [data]:
                            content = str(item)
                            if isinstance(item, bytes):
            ...
            # For 2D data, save as table
                ...
                output_path = self.output_dir / f"{safe_name}.dat"
                with open(output_path, 'w') as f:
                    f.write(f"# 2D data table ({data.shape[0]} x {data.shape[1]})\n")
                    np.savetxt(f, data, fmt='%g')
                return str(output_path)
    ...
    def extract_multiple_datasets(self, dataset_indices: List[int]) -> List[str]:
        """
        ...
        for i, idx in enumerate(dataset_indices):
            if 0 <= idx < len(self.datasets_info):
                ...
                print(f"  {Colors.CYAN}[{i+1}/{len(dataset_indices)}]{Colors.RESET} "
                      f"Extracting: {dataset_info['name']}")
                
                extracted_path = self.extract_dataset(dataset_info)
                if extracted_path:
                    extracted_files.append(extracted_path)
                    print(f"    {Colors.GREEN}✓ Saved to: {extracted_path}{Colors.RESET}")
                else:
        ...
        print(f"Successfully extracted {len(extracted_files)} out of {len(dataset_indices)} datasets")
    ...
    try:
        ...
        if args.extract:
            if args.extract.lower() == 'all':
                ...
                try:
                    selected_indices = [int(x.strip()) for x in args.extract.split(',')]
                except ValueError:
                    print(f"{Colors.RED}Error: Invalid dataset indices format{Colors.RESET}")
                    return 1
